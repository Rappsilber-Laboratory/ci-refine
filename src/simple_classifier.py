...
def stack_features(protein_data, data, train_indices, test_indices):
    ...
    for protein in test_proteins:
        ...
        if counter == 0:
            ...
            test_labels = list(itertools.chain(list(training_labels), list(labels)))
        counter += 1
...
def find_pagerank_parameters(test_proteins, alpha_range, beta_range, clf, shift_dict, all_values, clf_parameters, norm):
    ...
    for pdb_id, length in test_proteins:
        ...
        co_occurance_features_easy(contacts[0:int(int(length) * 1.5)], sec_struct, shift_dict, feature_matrix,
                                   contact_pairs)
...
def main():
    ...
    for train_indices, test_indices in kfold_split_iterator:
        ...
        training_features, training_labels, test_features, test_labels = stack_features(protein_data, data, train_indices,
                                                                                       test_indices)
    ...
    test_indices = []
    training_features, training_labels, _, _ = stack_features(protein_data, data, train_indices,
                                                                      test_indices)
    training_features = training_features.toarray()
