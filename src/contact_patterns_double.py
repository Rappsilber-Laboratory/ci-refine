...
def add_contacts( structure,  sec_struct_pair_types, shift_mat, sec_struct,sol):
    all_contacts = 0
    for i in xrange(13, structure.get_number_of_residues()+1-13):
        for j in xrange(i+1, structure.get_number_of_residues()+1-13):
            if abs(i-j) >= 12:
                distance = structure.get_contact_map().get_mapped_distance(i,j)
                if distance <= 8.0:
                    ...
                    for i_shift, j_shift in shift_mat:
                        if abs((i+i_shift)-(j+j_shift)) >= 1:
                            dist_shift = structure.get_contact_map().get_mapped_distance(i+i_shift,j+j_shift)
                            sec_2_lower = sec_struct[i+i_shift]
                            sec_2_upper = sec_struct[j+j_shift]
                            all_shifts = sec_struct_pair_types[(sec_lower,sec_upper, sec_2_lower, sec_2_upper)][0]
                            if dist_shift <= 8.5:
                                ...
                                sec_struct_pair_types[(sec_lower,sec_upper, sec_2_lower, sec_2_upper)] = ( all_shifts, all_contacts )
                    all_contacts += 1
    return all_contacts
...
def main():
    ...
    for i in xrange(0,len(sec_struct_types)):
        for j in xrange(0,len(sec_struct_types)):
            for k in xrange(0,len(sec_struct_types)):
                for l in xrange(0,len(sec_struct_types)):
                    all_shifts = {}
                    all_contacts = 0
                    sec_struct_pair_types[(sec_struct_types[i], sec_struct_types[j], sec_struct_types[k], sec_struct_types[l])] = (all_shifts, all_contacts)
...
#            print counts
   ...
   # print all_shifts
    ...
    """
    cPickle.dump(sec_struct_pair_types, open( "shifts_test_double.p", "wb" ),protocol=2 )
