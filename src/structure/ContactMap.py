...
class ContactMap:
  def __init__(self, contact_threshold = 8):
    self.contact_map = None#answer = numpy.zeros((len(chain_one), len(chain_two)), numpy.float)
  ...
  def draw_cm(self):
   ...
   # m = pylab.scatter(draw_map[0][:], draw_map[:][0],c="k", s="40")
    ...
    if self.fp_predictions != None:
        ...
        for x, y in self.fp_predictions:
            ...
            y_list.append(x-1)
        #m = pylab.scatter(x_list,y_list, c='#B40426', s=100,  linewidths=0.0, marker = (5,1))#3B4CC0
      #  m = pylab.scatter(x_list,y_list, c='#B40426', s=60,  linewidths=0.0, marker = (5,1))
