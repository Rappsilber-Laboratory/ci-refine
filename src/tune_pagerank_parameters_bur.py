...
from contact_page_rank_bur import build_ce_graph, do_page_rank
options = {}
...
def run_pagerank(xl_data, sec_struct, alpha, beta, shift_dict, input_length, bur):
    xl_graph, node_weights = build_ce_graph(xl_data, int(int(input_length) * beta), shift_dict, sec_struct, bur)
    xl_ranked = do_page_rank(xl_graph, node_weights, alpha, int(input_length))
    return xl_ranked

def run_pagerank_linear(xl_data, sec_struct, alpha, beta, gamma, shift_dict, input_length, bur):
    contact_dict = {}
    for score, i in xl_data:
        contact_dict[i] = score
    ...
    to_scale = []
    for _,_,score in xl_ranked:
        to_scale.append(score)

    norm.fit(to_scale)

    for i,j,score in xl_ranked:
        new_rank.append((i, j, gamma*norm.transform([score])[0] + (1-gamma)*contact_dict[(i,j)]))
...
def load_files_bur(pdb_id, sec_struct_folder=None, pred_folder=None, solv_folder=None):
    ...
    sec_struct = InputOutput.InputOutput.parse_psipred(psipred_file)
    bur_dict = InputOutput.InputOutput.parse_solpred(solv_file)
   
    xl_data = InputOutput.InputOutput.load_restraints_pr(contact_file, seq_sep_min=12)
    return xl_data, sec_struct, bur_dict
...
#        print keys, values
    ...
    for pdb_id, length in protein_data:
        ...
        contacts, sec_struct, bur_types = load_files_bur(pdb_id, sec_struct_folder=options.secondary_structure_folder,
                                                         pred_folder=options.prediction_folder,
                                                         solv_folder="/scratch/kstahl/test_sets/svmcon_test/solvpred/")
        #print contacts
        #print bur_types
        for alpha in alpha_range:
            for beta in beta_range:
                try:
                    xl_ranked = run_pagerank_linear(contacts, sec_struct, alpha, beta, 1.0, shift_dict, length, bur_types)
                    native_map = load_native_map(pdb_id)
