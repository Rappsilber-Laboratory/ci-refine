        ...
        for j in xrange(i+1, structure.get_number_of_residues()+1-9):
            if abs(i-j) >= 12:
                ...
                if distance <= 8.0:
                    ...
                    bur_lower = sol[i]
                    bur_upper = sol[j]
                    all_shifts = sec_struct_pair_types[(sec_lower,sec_upper,bur_lower,bur_upper)][0]
                    for i_shift, j_shift in shift_mat:
                    ...
                    all_contacts += 1
                    sec_struct_pair_types[(sec_lower,sec_upper, bur_lower, bur_upper)] = ( all_shifts, all_contacts )
    return all_contacts
...
def main():
    ...
    for i in xrange(0,len(sec_struct_types)):
        for j in xrange(0,len(sec_struct_types)):
            for k in xrange(0,len(bur_types)):
                for l in xrange(0,len(bur_types)):
                    all_shifts = {}
                    all_contacts = 0
                    sec_struct_pair_types[(sec_struct_types[i], sec_struct_types[j], bur_types[k], bur_types[l])] = (all_shifts, all_contacts)
    ...
    for line in file:
        ...
        bur_dict = InputOutput.InputOutput.parse_solpred("/scratch/kstahl/test_sets/rbo_test/solvpred/%s.solv"%pdb_id)
        print pdb_id
        try:
            tmp_struct.load_structure('xxxx', pdb_id[-1],pdb_file, seqsep =1)
        except:
