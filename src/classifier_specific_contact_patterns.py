...
def load_contact_data(pdb_id, leng):
    res_dict = {}
    restraints = InputOutput.InputOutput.load_restraints_pr("../data/predictor_results/epc-map_test/epc-map/%s.prediction"%pdb_id, seq_sep_min=12, max_contacts=int(int(leng)*1.5))
    for score, tuple in restraints:
        res_dict[tuple] = score
    return res_dict



def add_contacts( structure,  sec_struct_pair_types, shift_mat, sec_struct,sol, res_dict):
    all_contacts = 0
    for i in xrange(9, structure.get_number_of_residues()+1-9):
        for j in xrange(i+1, structure.get_number_of_residues()+1-9):
            if abs(i-j) >= 12:
                distance = structure.get_contact_map().get_mapped_distance(i,j)
                if distance <= 8.0 and res_dict.has_key((i, j)):
                    ...
                    for i_shift, j_shift in shift_mat:
                        if abs((i+i_shift)-(j+j_shift)) >= 1:
                            dist_shift = structure.get_contact_map().get_mapped_distance(i+i_shift,j+j_shift)
                            if dist_shift <= 8.5 and res_dict.has_key((i+i_shift,j+j_shift)):
                                if all_shifts.has_key((i_shift, j_shift)):
                                    if dist_shift <= 8.0:
                                        all_shifts[(i_shift, j_shift)] = all_shifts[(i_shift, j_shift)] + res_dict[(i+i_shift,j+j_shift)]
                                    ...
                                    all_shifts[(i_shift, j_shift)] = res_dict[(i + i_shift, j + j_shift)]
                    all_contacts += 1
...
def main():
    ...
    for line in file:
        pdb_id = str(line).strip().split()[0][0:5]
        leng = str(line).strip().split()[1]
        pdb_file = "/scratch/schneider/pdb_select_dataset/%s/%s.pdb"%(pdb_id[0:4],pdb_id)
        tmp_struct = StructureContainer.StructureContainer()
        sec_struct = ResidueFeatureSecStruct.ResidueFeatureSecStruct(pdb_file)
        sec_struct = sec_struct.ss_dict
        clean_sec_structs(sec_struct)
        res_dict = load_contact_data(pdb_id, leng)
        print pdb_id
        try:
            tmp_struct.load_structure('xxxx', pdb_id[-1],pdb_file, seqsep =1)
        except:
            tmp_struct.load_structure('xxxx', ' ',pdb_file, seqsep =1)
        all_contacts += add_contacts(tmp_struct, sec_struct_pair_types, shift_mat, sec_struct, None, res_dict )
   ...
   # print all_shifts
    ...
    """
    cPickle.dump(sec_struct_pair_types, open( "shifts_predictor.p", "wb" ),protocol=2 )
