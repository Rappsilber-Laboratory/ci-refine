...
def load_XL_result_data(result_file, seq_sep = 12):
...
y_true, y_predict = load_XL_result_data("analysis_results/HSA_20Perc_PSM_distances.txt")
n_top = int(len(y_true)*0.5)
print n_top
print accuracy(y_true, y_predict, n_top)
y_true_2, y_predict_2 = load_XL_result_data("analysis_results/HSA_20Perc_PR_distances.txt")
print accuracy(y_true_2, y_predict_2, n_top)
...
pylab.plot(fpr,tpr,'#B40426' , linewidth=1.0)
pylab.plot(fpr_2,tpr_2, '#3B4CC0' , linewidth=1.0)
...
pylab.ylim((0,1.00))
pylab.savefig("HSA_50Perc_ROC.svg",bbox_inches="tight",pad_inches=0.02,dpi=300)
