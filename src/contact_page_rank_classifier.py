...
import argparse
import cPickle
from compute_co_features import co_occurence_features, co_occurance_features_easy
...
def build_ce_graph(xl_data, length, shift_dict, sec_struct, contact_co_dict):
...
def do_page_rank(xl_graph, node_weights, input_alpha, input_len):
    ...
    This runs pagerank.
    """
    ranked_nodes = nx.pagerank(xl_graph, alpha=input_alpha, personalization=node_weights, max_iter=100, tol=1e-10)
...
def main():
    ...
    feature_matrix = []
    contact_pairs = []
    co_occurance_features_easy(xl_data[0:int(options.length * options.top)], sec_struct, shift_dict, feature_matrix, contact_pairs)


    with open('easy_model.clf', 'rb') as infile:
    ...
    xl_graph, node_weights = build_ce_graph(xl_data, int(options.length * options.top), shift_dict, sec_struct, contact_score_dict)
